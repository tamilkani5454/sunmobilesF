...
const OrderDetails = () => {
    ...
    useEffect(() => {
        const fetchOrderDetails = async () => {
            try {
                ...
                let ordersList = []
                if (data.success && data.order) ordersList = data.order
                else if (data.order) ordersList = data.order
                else if (Array.isArray(data)) ordersList = data
                
                const foundOrder = ordersList.find(o => o.orderId === id || o._id === id)
                setOrder(foundOrder)
            } catch (error) {
    ...
    if (!order) {
        return <div className="min-h-screen bg-gray-50 flex justify-center items-center">Order not found.</div>
    }

    // Adapt timeline status from actual order status
    const oStatus = order.orderStatus?.toLowerCase() || 'pending'
    const isCompleted = ['delivered', 'completed'].includes(oStatus)
    const isShipped = oStatus === 'shipped' || isCompleted
    const isProcessing = ['processing', 'placed', 'pending'].includes(oStatus) || isShipped || isCompleted
    
    const timeline = [
        { status: "Order Placed", date: order.createdAt ? new Date(order.createdAt).toLocaleString() : "Unknown", completed: true },
        { status: "Processing", date: isProcessing ? "Started" : "Pending", completed: isProcessing },
        { status: "Shipped", date: isShipped ? "Shipped" : "Pending", completed: isShipped },
        { status: "Delivered", date: isCompleted ? "Delivered" : "Pending", completed: isCompleted },
    ]
                ...
                <div className="mb-6 md:mb-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 flex flex-wrap items-center gap-2 md:gap-3">
                                ...
                                <span className={`px-2.5 py-1 rounded-full text-xs md:text-base font-bold uppercase tracking-wider border ${
                                    ...
                                    isProcessing ? 'bg-blue-50 text-blue-700 border-blue-100' :
                                    'bg-gray-100 text-gray-600 border-gray-200'
                                }`}>
                                    {order.orderStatus || 'Pending'}
                                </span>
                            </h1>
                            <p className="text-xs md:text-base text-gray-500 mt-1">Order ID: <span className="font-mono font-medium text-gray-700">#{order.orderId}</span> • Placed on {order.createdAt ? new Date(order.createdAt).toLocaleDateString() : ''}</p>
                        </div>
                        <button className="flex items-center justify-center gap-2 px-4 py-2 bg-white border border-gray-200 text-gray-700 text-sm md:text-base font-medium rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all shadow-sm">
                            <Download size={16} className="md:w-4.5 md:h-4.5" /> Download Invoice
                        </button>
                    </div>
                </div>
                                        ...
                                        <div key={idx} className="flex md:flex-col items-center md:text-center gap-4 md:gap-2">
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors duration-300 ${
                                                step.completed 
                                                    ...
                                                    : idx === 1 && isProcessing ? 'bg-blue-600 border-blue-600 text-white animate-pulse' // Current active step (simplified logic)
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            <div className="divide-y divide-gray-50">
                                {order.products && order.products.map((item, idx) => (
                                    <div key={item._id || idx} className="p-5 md:p-6 flex flex-col md:flex-row gap-4 md:gap-6 items-center">
                                        <div className="w-16 h-16 md:w-20 md:h-20 bg-gray-100 rounded-xl flex items-center justify-center shrink-0">
                                            <Package className="text-gray-400 w-8 h-8"/>
                                        </div>
                                        <div className="flex-1 text-center md:text-left">
                                            <h4 className="font-bold text-sm md:text-base text-gray-900 mb-1">Product ID: {item._id}</h4>
                                            <p className="text-xs md:text-sm text-gray-500 mb-2">Price per item: ₹{item.price?.toLocaleString()}</p>
                                        ...
                                        <div className="text-right">
                                            <p className="font-bold text-base md:text-lg text-gray-900">₹{(item.total || (item.price * item.quantity))?.toLocaleString()}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                         ...
                         <div className="grid md:grid-cols-2 gap-6">
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 md:p-6">
                                ...
                                <div className="space-y-1 text-xs md:text-sm text-gray-600">
                                    <p className="font-bold text-gray-900 text-sm md:text-base mb-1">{order.shippingAddress?.firstName} {order.shippingAddress?.lastName}</p>
                                    <p>{order.shippingAddress?.address}</p>
                                    <p>{order.shippingAddress?.city}, {order.shippingAddress?.state} - {order.shippingAddress?.pincode}</p>
                                    <p className="pt-2 flex items-center gap-2"><Phone size={12} className="md:w-3.5 md:h-3.5" /> {order.shippingAddress?.phoneNumber}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 md:p-6">
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <p className="font-medium text-sm md:text-base text-gray-900">{order.paymentMethod}</p>
                                    </div>
                                    <p className="text-xs md:text-sm text-gray-500">Payment Status: <span className="uppercase">{order.paymentStatus}</span></p>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 md:p-6 sticky top-24">
                            ...
                            <div className="space-y-3 md:space-y-4 mb-4 md:mb-6 pb-4 md:pb-6 border-b border-gray-50">
                                <div className="flex justify-between text-xs md:text-sm text-gray-600">
                                    <span>Subtotal</span>
                                    <span className="font-medium text-gray-900">₹{(order.finalAmount - (order.deliveryCharge || 0)).toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between text-xs md:text-sm text-gray-600">
                                    <span>Shipping</span>
                                    <span className={order.deliveryCharge ? "font-medium text-gray-900" : "text-green-600 font-medium"}>{order.deliveryCharge ? `₹${order.deliveryCharge.toLocaleString()}` : "Free"}</span>
                                </div>
                                <div className="flex justify-between text-xs md:text-sm text-gray-600">
                            ...
                            <div className="flex justify-between items-center mb-4 md:mb-6">
                                ...
                                <span className="font-bold text-orange-600 text-lg md:text-xl">₹{order.finalAmount?.toLocaleString()}</span>
                            </div>
                            <div className="bg-blue-50 rounded-xl p-3 md:p-4 flex items-start gap-3">
                                <ShieldCheck className="text-blue-600 shrink-0 w-4 h-4 md:w-5 md:h-5" />
                                <div>
                         ...
                         <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 md:p-6">
                            ...
                            <button className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-gray-200 text-gray-700 text-sm md:text-base font-medium rounded-xl hover:bg-gray-50 transition-colors mb-3">
                                <Mail size={16} className="md:w-4.5 md:h-4.5" /> Contact Support
                            </button>
                             <button className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-gray-200 text-gray-700 text-sm md:text-base font-medium rounded-xl hover:bg-gray-50 transition-colors">
                                <Package size={16} className="md:w-4.5 md:h-4.5" /> Return Items
                            </button>
                        </div>
                    </div>
                </div>
